...
function buildNotificationText(payload) {
  ...
  if (payload.changes && payload.changes.length) {
    ...
    const purchasable = (payload.rows || []).filter((row) => ["open", "limited"].includes(row.decisionStatus || row.status)).length;
    lines.push(`当前确认可申购记录：${purchasable} 条`, "额度与渠道无变化。");
  }
  return lines.join("\n");
}
...
function resolveWebhookUrl(options) {
  ...
  try {
    ...
    if (process.env.USER) args.push("-a", process.env.USER);
    args.push("-w");
    return childProcess.execFileSync("security", args, {
