...
function baseFundName(name) {
  ...
  if (/(?:ETF|LOF|FOF)$/i.test(normalized)) return normalized;
  return normalized.replace(/(?:人民币|美元现汇|美元现钞|美汇|美钞)?[A-Z](?:类)?(?:人民币|美元现汇|美元现钞|美汇|美钞|\((?:人民币|美元[^)]*)\))?$/i, "").trim();
}
...
function changeLine(change) {
  ...
  if (change.type === "amount-decreased") return `- 额度降低：${formatCurrencyAmount(change.before.limitAmount, currency)} -> ${formatCurrencyAmount(change.after.limitAmount, currency)}｜${prefix}`;
  if (change.type === "amount-changed") return `- 额度变化：${formatCurrencyAmount(change.before.limitAmount, currency)} -> ${formatCurrencyAmount(change.after.limitAmount, currency)}｜${prefix}`;
...
function groupLimitRows(rows) {
  ...
  sortPurchasableRows(rows).forEach((row) => {
    ...
    const route = row.route || "otc";
    const customChannel = row.channelType === "user-verified-channel" ? row.channel : "";
    const key = [row.index, amountKey, currency, route, customChannel, baseName].join("|");
    let displayName = baseName;
    if (route === "exchange") displayName += "（场内交易）";
    if (customChannel) displayName += `（${customChannel}）`;
    if (!groups.has(key)) groups.set(key, { amountKey, currency, displayName, codes: [] });
...
function renderLimitTable(lines, rows) {
  ...
  groupLimitRows(rows).forEach((group) => {
    const amount = group.amountKey === "open" ? "未显示上限" : formatCurrencyAmount(Number(group.amountKey), group.currency);
    lines.push(`| ${amount} | ${group.displayName} | ${group.codes.join("、")} |`);
  });
}
...
function renderDirectSaleSection(lines, evidence, officialNotices, indexName) {
  ...
  indexEvidence.forEach((row) => {
    const baseName = baseFundName(row.name);
    const key = [row.index, row.amount, row.currency, row.accountBasis || "", baseName].join("|");
