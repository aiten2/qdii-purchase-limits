...
脚本会自动完成公告查询，不得要求用户或 Agent 逐个打开网页。默认先查询中国证监会资本市场统一信息披露平台，再按目标基金代码查询公开公告索引，自动下载公告 PDF、核对份额代码并补齐统一披露时间线中遗漏的直销专属规则；基金管理人官网作为独立回退来源。公开公告索引只负责发现公告，基金管理人发布的公告正文才是规则依据。官方公告是申购规则的第一准则：最终可买必须同时满足官方允许和具体渠道允许，额度取该份额、该渠道适用公告与渠道页面中更严格的值。
...
`latest.json` 中的 `officialNotices.sources` 必须分别保存统一披露平台、公开公告索引和基金管理人官网的覆盖情况。公开公告索引未覆盖全部目标份额且其他来源没有补齐时，直销表必须提示数据尚未覆盖或暂未获取；主清单仍只使用已完成公告核验的结果。
...
可用 `--channels FILE` 合并人工或其他自动化核验的渠道记录。记录必须有失效时间，过期数据不会参与“当前确认可买”：
